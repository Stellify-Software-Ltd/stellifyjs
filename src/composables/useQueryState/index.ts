 ...
 */
export interface QueryStateOptions<T = unknown> {
  /**
 ...
 * Full schema type
 */
type QuerySchema = Record<string, QueryStateOptions<unknown> | ShorthandConfig<unknown>>

/**
 * Extract the value type from a config
 */
type ExtractValueType<C> = C extends QueryStateOptions<infer T>
  ? T
  : C
 ...
 */
export type QueryStateReturn<S extends QuerySchema> = {
 ...
 */
const normalizeConfig = <T>(config: QueryStateOptions<T> | T): QueryStateOptions<T> => {
  if (
    typeof config === 'object' &&
    config !== null &&
    'default' in config
  ) {
    return config as QueryStateOptions<T>
  }
...
export function useQueryState<S extends QuerySchema>(
  ...
  const configs: Record<string, QueryStateOptions<unknown>> = {}
